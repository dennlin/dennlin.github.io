...
const Logo = () => {
  ...
  useEffect(() => {
    ...
    // Start by fading in the background
    tl.to(bgRef.current, { duration: 1, opacity: 1 });
  
    // Animate the drawing of the SVG path. Adjust this duration to match the actual drawing time.
    tl.fromTo(outlineLogoRef.current, { drawSVG: "0%" }, { drawSVG: "100%", duration: 20 });
  
    // Start fading in the solid logo immediately after the drawing animation
    tl.fromTo(solidLogoRef.current, { opacity: 0 }, { opacity: 1, delay: 4, duration: 4 });
  }, []);
